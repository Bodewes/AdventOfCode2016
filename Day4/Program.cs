...
namespace Day4
{
    class Program
    {
        ...
        {
            System.Console.WriteLine(new Romm().countRealRooms("aaaaa-bbb-z-y-x-123[abxyz]"));
            System.Console.WriteLine(new Romm().countRealRooms("a-b-c-d-e-f-g-h-987[abcde]"));
            System.Console.WriteLine(new Romm().countRealRooms("not-a-real-room-404[oarel]"));
            System.Console.WriteLine(new Romm().countRealRooms("totally-real-room-200[decoy]"));
            System.Console.WriteLine(new Romm().countRealRooms("qzmt-zixmtkozy-ivhz-343[zimth]"));
            #region input
            System.Console.WriteLine(new Romm().countRealRooms(@"bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
hplazytkpo-prr-cpnptgtyr-379[prtya]"));
    ...
    public class Romm
    {
        public int countRealRooms(string input)
        {
            var count = 0;
            var lines = input.Split('\n');
            foreach (var line in lines)
            {
                count += realRoomValue(line);
            }
            return count;
        }

        private int realRoomValue(string line)
        {
            var lastDash = line.LastIndexOf('-');
            var checksumIndex = line.IndexOf('[');
            var roomName = line.Substring(0, lastDash);
            var sectorID = int.Parse(line.Substring(lastDash + 1, checksumIndex - lastDash-1));
            var checksum = line.Substring(checksumIndex + 1, 5);

            if (checkRoom(roomName, checksum))
            {
                writeRealRoomName(roomName,sectorID);
                return sectorID;
            }
        ...
        private void writeRealRoomName(string roomName, int sectorID)
        {
            ...
            foreach (var c in roomName)
            {
                if (c == '-') newName += " ";
                    ...
                    newName += Convert.ToChar( (((c - 'a') + move) % 26) + 'a');
            }
            if (newName.Contains("north"))
            {
                System.Console.WriteLine(newName + " -> " + sectorID);
            }
            
        }

        private bool checkRoom(string roomName, string checksum)
        {
            var lettersHistogram = new SortedDictionary<char, int>();
            for (var c = 'a'; c <= 'z'; c++)
            {
                lettersHistogram.Add(c, 0);
            }
            ...
            var top5 = lettersHistogram.ToList().OrderByDescending(x => x.Value).Take(5);
            //var s = from kv in lettersHistogram orderby kv.Value, kv.Key select kv.Key;
            //var top5 = s.Take(5);
            var s = "";
