...
namespace Day7
{
    class Program
    {
        ...
        {
            System.Console.WriteLine(new TLStest().countTLSSupport("abba[mnop]qrst"));
            System.Console.WriteLine(new TLStest().countTLSSupport("abcd[bddb]xyyx"));
            System.Console.WriteLine(new TLStest().countTLSSupport("aaaa[qwer]tyui"));
            System.Console.WriteLine(new TLStest().countTLSSupport("ioxxoj[asdfgh]zxcvbn"));
            System.Console.WriteLine(new TLStest().countTLSSupport(TLStest.input));

            System.Console.WriteLine(new SSLTest().countSSLSupport("aba[bab]xyz"));
            System.Console.WriteLine(new SSLTest().countSSLSupport("xyx[xyx]xyx"));
            System.Console.WriteLine(new SSLTest().countSSLSupport("aaa[kek]eke"));
            System.Console.WriteLine(new SSLTest().countSSLSupport("zazbz[bzb]cdb"));
            System.Console.WriteLine(new SSLTest().countSSLSupport(TLStest.input));
    ...
    public class SSLTest
    {
        ...
        public int countSSLSupport(string input)
        {
            var lines = input.Split('\n');
            return lines.Count(x => supportsSSL(x));
        }
        ...
        public bool supportsSSL(string input)
        {
            ...
            for (int j = 0; j < segments.Length; j = j + 2)
            {
                ABAs.AddRange(getABA(segments[j]));
            }

            foreach (var a in ABAs)
            {
                for (int k = 1; k < segments.Length; k = k + 2)
                {
                    if (containsBAB(segments[k],a ))
                    {
                        return true;
                    }
                }
        ...
        public List<string> getABA(string input)
        {
            ...
            for (var i = 0; i < input.Length - 2; i++)
            {
                if (input[i] == input[i+2] &&
                    input[i] != input[i+1]){
                        result.Add(input.Substring(i + 1, 2) + input[i+1]);
                }
        ...
        public bool containsBAB(string input, string BAB)
        {
    ...
    public class TLStest
    {
        ...
        public int countTLSSupport(string input){
            var lines = input.Split('\n');
            return lines.Count(x=> supportTLS(x));
        }
        


        public bool supportTLS (string input){
            var segments = input.Split(new char[]{'[',']'});
            var outside = false;
            for (int j = 0; j < segments.Length; j = j + 2)
            {
                if (containsABBA(segments[j]))
                {
                    outside = true;
                    break;
                }
        ...
        public  const string input = @"dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
